...
import { adminService, officerService } from '../../api/services'
...
export default function OfficerStudents() {
  ...
  useEffect(() => {
    ...
    if (filterStatus !== 'ALL') data = data.filter(s => s.placementStatus === filterStatus)
    setFiltered(data)
  }, [search, filterDept, filterStatus, students])

  const toggleEligibility = async (id, eligible) => {
    try {
      await officerService.updateEligibility(id, eligible)
      setStudents(prev => prev.map(s => s.userId === id ? { ...s, eligible } : s))
      toast.success(`Student ${eligible ? 'marked eligible' : 'marked ineligible'}`)
    } catch { toast.error('Failed to update eligibility') }
  }

  const depts = ['ALL', ...new Set(students.map(s => s.department).filter(Boolean))]
  const placementStatuses = ['ALL','NOT_PLACED','PLACED','INTERNSHIP','HIGHER_STUDIES']
  ...
  return (
    <DashboardLayout>
      ...
      <div style={{ display:'flex', gap:12, marginBottom:24, flexWrap:'wrap' }}>
        ...
        <select className="form-input" style={{ width:180 }} value={filterDept} onChange={e => setFilterDept(e.target.value)}>
          {depts.map(d => <option key={d} value={d}>{d === 'ALL' ? 'All Departments' : d}</option>)}
        </select>
        <select className="form-input" style={{ width:180 }} value={filterStatus} onChange={e => setFilterStatus(e.target.value)}>
          {placementStatuses.map(s => <option key={s} value={s}>{s === 'ALL' ? 'All Statuses' : s.replace(/_/g,' ')}</option>)}
        </select>
      </div>
        ...
        : (
          <div className="card">
            <div className="table-wrap">
              <table>
                <thead>
                  <tr><th>Student</th><th>Roll No</th><th>Dept</th><th>Batch</th><th>CGPA</th><th>Status</th><th>Eligible</th><th></th></tr>
                </thead>
                <tbody>
                  {filtered.map(s => (
                    <tr key={s.userId}>
                      ...
                      <td>
                        {s.placementStatus === 'PLACED'
                          ...
                          : <span className="badge badge-gray">{s.placementStatus?.replace(/_/g,' ') || 'N/A'}</span>}
                      </td>
                      <td>
                        {s.eligible
                      ...
                      <td>
                        <button
                          className={`btn btn-sm ${s.eligible ? 'btn-danger' : 'btn-success'}`}
